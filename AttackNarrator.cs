...
namespace Cyber_behaviour_profiling
{
    ...
    public static class AttackNarrator
    {
        ...
        public static void PrintNarrative(AttackNarrative narrative)
        {
            ...
            ConsoleColor gradeColor = narrative.Grade switch
            {
            ...
            Console.Write($"{narrative.ProcessName}  PID:{narrative.ProcessId}");
            if (narrative.TotalSeconds > 0)
                Console.Write($"  ({narrative.TotalSeconds:F1}s)");
            Console.WriteLine();

            Console.ForegroundColor = gradeColor;
            Console.WriteLine($"── {narrative.Grade} {new string('─', 60)}");
            Console.ResetColor();
            Console.WriteLine();
            ...
            var displaySteps = narrative.Timeline.ToList();
            ...
            else
            {
                ...
                foreach (var step in displaySteps.OrderBy(s => s.Timestamp))
                {
                    ConsoleColor color = step.Category switch
                    {
                        "File"     => ConsoleColor.White,
                        "Registry" => ConsoleColor.DarkYellow,
                        "Network"  => ConsoleColor.Red,
                        "Process"  => ConsoleColor.Magenta,
                        "Info"     => ConsoleColor.DarkGray,
                        _          => ConsoleColor.Gray
                    };

                    Console.ForegroundColor = color;
                    string timeOffset = narrative.TotalSeconds > 0
                        ? $"+{(step.Timestamp - narrative.FirstSeen).TotalSeconds:F2}s"
                        : "";
                    Console.WriteLine($"    {step.Icon} {timeOffset,-10} {step.Headline}");

                    if (!string.IsNullOrEmpty(step.Detail) && step.Detail != step.Headline)
                    {
                        Console.ForegroundColor = ConsoleColor.DarkGray;
                ...
                Console.WriteLine();
                Console.ForegroundColor = gradeColor;
                Console.WriteLine($"  Verdict: {narrative.OverallStory}");
                Console.ResetColor();
            }

            if ((narrative.Grade == "SUSPICIOUS" || narrative.Grade == "INCONCLUSIVE") && narrative.AbsentIndicators.Any())
            {
                Console.WriteLine();
                Console.ForegroundColor = ConsoleColor.DarkGreen;
                Console.WriteLine("  What was NOT observed:");
                foreach (var absent in narrative.AbsentIndicators)
